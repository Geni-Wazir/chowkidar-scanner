    ...
    if 'interesting_findings' in data:
        for finding in data['interesting_findings']:
            ...
            if 'xmlrpc' in finding['type']:
                wordpress_vulns[name.title()] = [finding['url'], finding['references']]
            if 'debug_log' in finding['type']:
                wordpress_vulns[name.title()] = [finding['url']]

def wordpress_version(data):
    worpress_vulnerability = []
    if data['version']['status'] in ['outdated', 'insecure']:
        wordpress_vulns['Outdated Wordpress Version Being Used'] = [data['version']['number']]
    if data['version']['vulnerabilities'] != []:
        ...
        wordpress_vulns['Wordpress Version With Known Vulnerabilities'] = worpress_vulnerability
    ...
    if 'themes' in data:
        for theme in data['themes']:
            if data['themes'][theme]['outdated']:
                ...
                wordpress_vulns['Outdated Wordpress Themes Being Used'] = outdated_themes
            if data['themes'][theme]['vulnerabilities'] != []:
                ...
                wordpress_vulns['Wordpress Theme With Known Vulnerabilities'] = vulnerable_theme
    else:
    ...
    if 'plugins' in data:
        ...
        for plugin in data['plugins']:
            ...
            if data['plugins'][plugin]['outdated']:
                ...
                wordpress_vulns['Outdated Wordpress Plugins Being Used'] = outdated_plugins
            if data['plugins'][plugin]['vulnerabilities'] != []:
                ...
                wordpress_vulns['Wordpress Plugin With Known Vulnerabilities'] = vulnerable_plugin

def wordpress_config_backups(data):
    if 'config_backups' in data:
        config_backups = list(data['config_backups'].keys())
        if config_backups != []:
            wordpress_vulns['Wordpress Config Backup File Discovered'] = config_backups

def wordpress_users(data):
    if 'users' in data:
        users = list(data['users'].keys())
        if users != []:
            wordpress_vulns['Wordpress Users Discovered'] = users
